...
{
 var name=document.getElementById("qty"+id).innerHTML;


 document.getElementById("qty"+id).innerHTML="<input type='text' class='form-control' id='name_val"+id+"' value='"+name+"'>";
...
function save_row(id)
{
 ...
 ({
  type:'post',
  url:'control/modify.penjualan.php',
  data:{
   edit_row:'edit_row',
   name:name,
   row_id:id
  ...
  success:function(response) {
   if(response=="success")
   {
    document.getElementById("qty"+id).innerHTML=name;
